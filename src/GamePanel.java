...
URDTimeMillis = (System.nanoTime() - startTime)/1000000;
		 ...
		 for (int i = 0; i < enemies.size();i++)
	        {
	            ...
	            for (int j = 0; j < bullets.size(); j++)
	            {
	                ...
	                if(remove)
	                {
	                    	...
	                    	else if (rand < 0.030) { powerups.add(new PowerUp(3,ex,ey));}
	                    	else if (rand < 0.130) { powerups.add(new PowerUp(4,ex,ey));}
	...
	private void gameRender() 
	{
		...
		g.drawString("FPS: "+Math.round(averageFPS), 0, 13);
		g.drawString("Bullets lvl: " +player.getPower()+" / "+player.getMaxPowerLevel(), 0, 26);
		...
		{
			g.setColor(Color.WHITE);
			g.fillOval((20 * i) + player.getR() / 2, 32, player.getR() + 5, player.getR() + 5);
